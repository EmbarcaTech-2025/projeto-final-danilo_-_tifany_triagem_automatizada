...
// helper replacement for Arduino min
...
void maxim_heart_rate_and_oxygen_saturation(uint32_t *pun_ir_buffer, int32_t n_ir_buffer_length,
  ...
  for (k = 0; k < n_ir_buffer_length; k++) an_x[k] = -1 * ((int32_t)pun_ir_buffer[k] - (int32_t)un_ir_mean);
  ...
  if (n_npks >= 2) {
    ...
    *pn_heart_rate = (int32_t)((FreqS * 60) / my_min(n_peak_interval_sum, 32000));
    *pch_hr_valid = 1;
  } else {
    *pn_heart_rate = -999;
    *pch_hr_valid = 0;
